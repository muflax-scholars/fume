...
COMMAND is one of:
  ...
  * week - Shortcut for display with start date set to monday of this week.
  ...
  Submit bugs and feature requests to http://github.com/muflax/fumetrap/issues
    EOF
    ...
    def set_global_options
      Fumetrap::Entry.round = true if args['-r']
    end
    ...
    def out
      ...
      if Timer.stop sheet, args['-a']
        duration = Timer.entries(sheet).last.duration
        warn "Checked out of sheet #{sheet.inspect} (#{format_out duration})."
      else
    ...
    def list
      sheets = ([Timer.current_sheet] | Entry.sheets).map do |sheet|
        ...
        entries = Fumetrap::Entry.filter(:sheet => sheet)
        if entries.empty?
    ...
    def week
      args['-s'] = Date.today.wday == 1 ? Date.today.to_s : Date.parse(parse_time("last monday").to_s).to_s
      display
    end
